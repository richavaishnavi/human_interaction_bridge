...
def listen_and_transcribe_sr(offline: bool = False, timeout: int = 5) -> str:
    ...
    recognizer.pause_threshold = 1.2 # <-- ADD THIS LINE recognizer.dynamic_energy_threshold = True # <-- AND THIS ONE
...
def listen_and_transcribe_whisper(timeout: int = 5) -> str:
    ...
    recognizer = sr.Recognizer()
    recognizer.pause_threshold = 1.2          # <-- ADD THIS LINE
    recognizer.dynamic_energy_threshold = True # <-- AND THIS ONE
    with sr.Microphone() as source:
        print("Adjusting for ambient noise... please wait.")
        recognizer.adjust_for_ambient_noise(source, duration=1)
        print("Listening... speak your sentence now.")
