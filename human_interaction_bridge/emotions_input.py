...
emotion_input.py
--------------------
Reads the webcam and detects ONLY the facial emotion (gesture detection
has been removed).
...
class EmotionInputDetector:
    """
    Wraps the face landmarker, reading from the webcam each time.
    """
    ...
    def __init__(self):
        ...
        # Smoothing buffer — avoids emotion flicker
    ...
    def read_frame_and_detect(self, debug=False):
        ...
        Reads one frame from the webcam and returns:
            frame (for displaying), emotion (str)
        """
        ...
        return frame, self.stable_emotion
...
# For app.py, it's better to create ONE EmotionInputDetector and keep
...
def get_current_input():
    ...
    detection stabilize, returns one {"emotion": ...} result,
    then closes the webcam again.
    """
    detector = EmotionInputDetector()
    result = {"emotion": "Neutral"}
    for _ in range(15):  # read a few frames so smoothing has data to work with
        frame, emotion = detector.read_frame_and_detect()
        result["emotion"] = emotion
    detector.close()
    return result


# ---------- Run this file directly to see the detection live ----------
...
if __name__ == "__main__":
    ...
    try:
        while True:
            frame, emotion = detector.read_frame_and_detect(debug=False)
            if frame is None:
            ...
            cv2.imshow("Emotion Input", frame)
